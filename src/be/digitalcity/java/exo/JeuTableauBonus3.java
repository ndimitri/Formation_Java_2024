...
public class JeuTableauBonus3 {
    public static void main(String[] args) {
        ...
        while (nbrMursPlaces < NOMBRE_MURS){
            int xMur = random.nextInt(0,10);
            int yMur = random.nextInt(0,10);
            ...
            switch(choixJoueur) {
                case 'd' -> {
                    ...
                    if(colonnePion == TAILLE_TABLEAU-1){
                        //Si il y a un mur on ne deplace pas le pion
                        if(plateau[lignePion][0]== '#'){
                            System.out.println("Il y a un mur à cet endroit !!! Vous ne pouvez pas y aller !!!");
                        }
                        else {
                            if(plateau[lignePion][0] == '$'){
                                compteurPieces++;
                ...
                default -> {
                    System.out.println("Cette direction n'existe pas, choississez une direction (g, d, h, b)");
                }
            }
